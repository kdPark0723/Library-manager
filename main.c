...
Borrow *create_borrow(Client const *client, Book const *book) {	
	...
	struct tm *t;
	t= localtime(&now);
	wcscpy(borrow.student_number,client->student_number);
	wcscpy(borrow.book_number,book->number);
	borrow.loan_date=now;
	if ((t->tm_wday+30)/7==0)//(t->tm_wday+30)/7==30일 뒤의 요일
		borrow.return_date=now+31*24*60*60;
	else
		borrow.return_date=now+30*24*60*60;
	return borrow_p; 
}
...
LinkedList *insert_client(LinkedList const *client_list, const Client const *client) {
	...
	LinkedList *new_p = (Linked List *)malloc(sizeof(LinkedList));
	int c;
	while (current != NULL){
